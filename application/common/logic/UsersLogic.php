...
//            $comment_count   = M('order_goods')->where("order_id", $add['order_id'])->where('is_comment', 0)->count();
    ...
    public function get_curr_time_section()
    {
        ...
        // $order = DB::name('OrderSignReceive')->where(['uid'=>user_id, 'type'=>2])->where('addend_time','>=',$timeBegin)->where('addend_time','<=',$timeEnd)->find();
		//最后更新‘统计’时间

        if ($current > $user['count_time'] && $user['level'] == 2 /*&& $order != null*/){

            $num = M('UserLevel')->where('level',$user['level'])->value('receive_num');
    ...
    public function update_receipt_num()
    {
        ...
        if ($current > $user['count_time'] && $user['level'] >= 3){
            ...
            $order = Db::name('OrderSignReceive')->where(['uid'=> $user_id])->where('addend_time','>=',$timeBegin)->where('addend_time','<=',$timeEnd)->sum('goods_num');

            $num = M('UserLevel')->field('get_num,receive_num')->where('level',$user['level'])->find();
